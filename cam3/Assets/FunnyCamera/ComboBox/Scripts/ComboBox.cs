...
public class ComboBox : MonoBehaviour 
{
	...
	public Color Sprite_UIColor = Color.white;
	public Sprite Sprite_Background;
	public Color Sprite_BackgroundColor= Color.white;
	public Color scrollerbarBackgroundColor = Color.white;
	public Color scrollerbarHandleColor = Color.white;
	public Vector2 itemSpace;
	public float e_scrollbarWidth = 12.0f;

	[SerializeField][HideInInspector]
	private DirectionEnum m_DirectionMode;
	public DirectionEnum DirectionMode
	{
		get
		{
	...
	public void CreateControl()
	{
		...
		comboButtonImage.color = Sprite_UIColor;
		...
		comboButtonColors.normalColor = new Color32(255, 0, 0, 128);
	...
	public void InitControl()
	{
		...
		scrollPanelImage.color = scrollerbarBackgroundColor;
		...
		if (m_DirectionMode == DirectionEnum.DOWN) {
			scrollPanelRectTransfrom.anchoredPosition = new Vector2 (0.0f, -comboButtonRectTransform.sizeDelta.y);
		}
		else if(m_DirectionMode == DirectionEnum.UP) {
			scrollPanelRectTransfrom.anchoredPosition = new Vector2 (0.0f, comboButtonRectTransform.sizeDelta.y * _itemsToDisplay);
		}

		if (m_DirectionMode == DirectionEnum.UP) {
			cba.GetComponent<Text>().text = "▲";
		} else if (m_DirectionMode == DirectionEnum.DOWN) {
			cba.GetComponent<Text>().text= "▼";
		}
		...
		var scrollbarWidth = Items.Length - (HideFirstItem ? 1 : 0) > _itemsToDisplay ? e_scrollbarWidth : 0.0f;
		...
		itemsGridLayoutGroup.spacing = itemSpace;
		...
		scrollbarImage.color = scrollerbarBackgroundColor;
		...
		handleImage.color = scrollerbarHandleColor;
	...
	public void updateTheArrowText()
	{
	...
	public void Refresh()
	{
		...
		var scrollbarWidth = itemsLength > ItemsToDisplay ? e_scrollbarWidth : 0.0f;
		scrollPanelRectTransfrom.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, dropdownHeight);
		scrollbarRectTransfrom.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, scrollbarWidth);
		scrollbarRectTransfrom.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, dropdownHeight);
		slidingAreaRectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, dropdownHeight - scrollbarRectTransfrom.sizeDelta.x);
	...
	public void UpdateGraphics()
	{
		if (overlayGO != null)
		{
			var scrollbarWidth = Items.Length - (HideFirstItem ? 1 : 0) > ItemsToDisplay ? e_scrollbarWidth : 0.0f;
